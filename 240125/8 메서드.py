# 메서드(메소드)
# 메서드 method란 특정 객체 object가 가지고 있는 함수 funtion을 의미
# 함수는 독립적으로 호출할 수 있지만, 메서드는 특정 객체를 통해서만 호출이 가능하다.

# 함수와 다르게 메서드는 특정 객체 소속이어서, 메서드를 호출하려면
# 특정 객체를 통해서만 호출 가능

# 문자열 메서드
# 문자열 str을 처리하기 위해 많은 메서드를 제공
...
# 인덱스를 지정해서 범위를 지정할 수도 있다.
s = 'best of best'
print(s.count('best', 8))
...
# 시작할 인덱스를 지정하지 않은 경우에는 문자열의 처음부터 찾고, 시작할 인덱스를 지정하는 경우 지정된 인덱스부터 찾음
...
# 4) index()
# find() 메서드와 같음. 사용방법도 같음. 차이점은 문자열이 없을 때 발생
# find 메서드는 찾는 문자열이 없는 경우 -1 반환, index() 메서드는 오류 발생
...
a = {'a': 'apple', 'b': 'banana'}
print(''.join(a))  # ab / 딕셔너리의 경우 key값을 연결시킨다.

# 7) split()
# 하나의 문자열을 여러 개의 문자열로 분리해서 저장한 리스트를 반환
print()
s = 'Life is too short'
s2 = s.split()  # 메서드에 아무것도 적지 않으면 공백문자를 기준으로 각 문자열이 분리
print(s)
print(s2)
