...
# 자동으로 데이터 라벨(0~4) 부여되고, 세로줄 라벨을 index라고 하고, 시퀀스 데이터는 value라고 한다.

# Series 데이터는 index와 value를 분리해서 가져올 수 있다.
# Series 데이터를 s라고 할 때 index는 s.index로, value는 s.value로 가져올 수 있다.
print(s1.index)
print(s1.values)
...
# 변수 s2는 문자열과 숫자가 혼합되어 있어 타입이 object = 객체이다.

# 데이터가 없는 경우 numpy를 임포트 한 후 np.nan으로 데이터가 없음을 표시할 수 있다.
import numpy as np

a3 = pd.Series([np.nan, 10, 30])
print(a3)
...
# 어느 가게의 날짜별 판매량을 pandas의 Series 형식으로 입력. 하루는 판매를 하지 않아 np.nan으로 입력

index_data = ['2024-02-05', '2024-02-06', '2024-02-07', '2024-02-08']
...
# 두 개의 DataFrame 데이터 df1과 df2는 길이가 다르다
# 길이가 같이 않더라도 연산이 가능
print(df1 + df2)

# Series()와 마찬가지로 DataFrame도 연산할 수 있는 항목 끼리만 연산하고, 그렇지 못하면 NaN으로 표시
...
# describe()를 이용하면 평균, 표준편차, 최솟값, 최댓값 등을 한 번에 구할 수 있다.
# 25% / 50% / 75% : 백분위수의 각 지점으로 분표를 반영해 평균을 보완하는 목적으로도 사용
print(df3.describe())
