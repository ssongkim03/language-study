...
# 장점
# 단순하다, 많은 프로그램에서 지원을 하고(Python, Visual code, Eclipse), 텍스트 편집기에서 볼 수 있으며, 스프레드 시트 데이터를 표현하는 간단한 방법
...
# csv 모듈을 사용해서 csv 파일을 읽으려면 다른 텍스트 파일처럼  open() 함수를 써도 가능
# example_file = open('./input/회원명단.csv')
#
# # read() 함수 대신 csv.reader() 함수에 전달. reder() 객체가 반환
#
# example_file = csv.reader(example_file)
#
# # list로 변환하여 접ㅈ근
# # print(example_reader) # 객체의 주소값
# example_data = list(example_reader)


# 2. for 반복문을 활용해 reader 객체로부터 데이터 읽기
# CSV 파일이 큰 경우에는 전체 파일을 한 번에 메모리에 로드(읽지) 않고
...
#     print(f'{str(example_readerq.line_num)} {str(row)} {str(row[2])}')

# 4. 키보드 인자 delimiter와 lineterminator
# delimiter : 구분자, 설명해서 넘어감
# lineterminator : 끝문자 : 파일에 쓸 때 각 행의 종료를 나타내는 제어 문자열로 적용
# 셀들이 탭으로 구분되어 있으면 탭으로 구분된 값을 의미하는 .tsv 파일 확장자를 사용
...
# DictReader 나 DictWriter 객체를 사용하는 것이 작업하기 편하다
...
# 파일에 헤더 행을 넣고 싶으면 writerheader() 호출
output_dict_writer.writeheader()
# writerow() 메서드를 호출하여 각 행을 쓸 수 있는데 이 때 딕셔너리를 사용
# 딕셔너리의 키는 헤더(filenames)가 되고, 딕셔너리의 값(value)는 파일에 쓰려는 데이터가 들어가는 형식
output_dict_writer.writerow({'Name' : 'Alice', 'Pet' : 'Cat', 'Phone' : '010-1234-5678'})
output_dict_writer.writerow({'Phone' : '010-1234-5678', 'Name' : 'Bob'})
output_dict_writer.writerow({'Pet' : 'Dog', 'Name' : 'Daniel', 'Phone' : '010-1234-5678'})
