...
# 2. SCV 파일 읽기
# 장점 : 현재 한국의 전자정부 프레임 워크, 데이터를 기록할 때 99프로 .csv 파일로 저장한다.
# 단점 : 한국, 일본밖에 사용 안 함.

# 1) 한 줄에 한 데이터가 있기 때문에 readline() 메소드로 한 줄씩 읽는다.
...
#             break
#         student = line.split(',')
#         # .split()는 문자열을 특정 구분자를 기준으로 나누어 리스트 형태로 변환
#         print(student)
#         student_list.append(student)
# print(student_list)

# 학생 1명의 정보인 학번, 이름, 주소, 연락처를 student 리스트에 저장하고, student_list에 student 리스트를 저장

# CSV 파일을 사용하다 보면 간혹 큰 따옴표(")를 이용해서 텍스트를 묶는 경우가 있음.
# 큰 따옴표를 제거하기 위해 추가로 strip () 메서드를 사용

# with : 일반적으로 파일을 열거나 DB연결을 생성하고 사용하는 경우 자원을 자동으로 관리하고, 예외가 발생해도 적절하게 해제해주는 장점이 있다.
...
#             break
#         member = line.split(',')
#         member[0] = member[0].strip('"') # 큰 따옴표 제거
...
# w 모드로 열고 생성된 파일 객체를 csv.writer() 메서드에 전달
# 그러면 csv 파일을 생성할 수 있는 객체가 생성되는데, 이 객체를  이용하여 writer() 메서드를 호출하면 csv 파일에 데이터를 저장할 수 있다.
...
#         csv_maker.writerow([1, '08러 1234', '2024-02-06, 13:43'])
#         csv_maker.writerow([2, '25다 123', '2024-01-01, 00:00'])
#         csv_maker.writerow([3, '28하 1234', '2024-02-09, 16:30'])
...
with open('./output/차량관리_01.csv','w',newline='') as file:
    csv_maker = csv.writer(file, delimiter=',')
    csv_maker.writerow([1, '08러 1234', '2024-02-06, 13:43'])
    csv_maker.writerow([2, '25다 123', '2024-01-01, 00:00'])
    csv_maker.writerow([3, '28하 1234', '2024-02-09, 16:30'])
