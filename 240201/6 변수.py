...
# 그러나 프로그램을 종료한 후에도 데이터를 유지하고 싶은 경우, 파일 형태로 저장해야 한다.

# 1. 파일과 파일 경로
# 파일에는 파일 이름과 경로라는 두 가지 주요속성이 있다.

# 예를 들어 윈도우 운영체제를 사용하는 컴퓨터의 경우
# C: \파이썬2(방특)\파이썬2\2024=02-01이라는 경로에 6. 변수.py 파일이 있는 경우
# 파일이름 6. 변수.py 파일이고 파이썬2, 파이썬2(방특), 2024-02-01 은 모두 폴더(디렉터리 or 디렉토리)라고 한다.
# 폴더 안에는 파일이나 다른 폴더가 들어 있을 수 있다.
# 경로의 C:\는 루트 폴더를 나타내며 모든 폴더가 이 안에 있다.
# 윈도우에서 루트 폴더의 이름은 "C:\"이며 C드라이브 라고 부른다.
# 맥OS나 리눅스에서 루트 폴더는 "\"
# 윈도우, 맥OS에서는 폴더이름이나 파일이름의 대소문자를 구분하지 않지만,
# 리눅스는 구분한다.

# 윈도우에서 폴더 구분은 "\"백 슬러시를 사용하고, 맥OS나 리눈스에서는 "/" 슬러시를 사용한다.
# 프로그램의 모든 운영체제에서 작동하게 하려면 "\", "/" 두 가지 경우를 모두 처리하도록 코드를 작성하는 것이 좋다.
